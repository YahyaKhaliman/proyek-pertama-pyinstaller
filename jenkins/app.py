...
# Cari tahu lokasi proyek
project_path = "/var/jenkins_home/workspace/simple-node-js-react-app"

# Build the Node.js/React application
print("Building the Node.js/React application...")
subprocess.call(["ls", project_path])  # Cek direktori proyek
subprocess.call(["npm", "run", "build"], cwd=project_path, shell=True)

# Start the Node.js/React application in development mode
print("Starting the Node.js/React application in development mode...")
start_process = subprocess.Popen("npm start &", cwd=project_path, shell=True, preexec_fn=os.setsid)

# Wait for a moment to ensure the application is up and running
time.sleep(1)

# Write the process ID (PID) to a file
with open(project_path + "/.pidfile", "w") as pidfile:
